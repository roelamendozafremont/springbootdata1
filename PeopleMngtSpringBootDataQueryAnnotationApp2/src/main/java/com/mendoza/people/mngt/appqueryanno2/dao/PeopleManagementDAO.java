...
public interface PeopleManagementDAO extends CrudRepository<Person, String> {
	...
	@Query(value="SELECT * from persontable WHERE pfathersname LIKE ?1",nativeQuery=true) 
	List<Person> findLikefathersname(String fathersname);	
	
	@Query(value="SELECT * from persontable WHERE pfathersname LIKE ?1 AND pmothersname LIKE ?2",nativeQuery=true)
